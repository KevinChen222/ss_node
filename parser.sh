...
_parse_vless() {
    ...
    if [[ $link =~ $regex ]]; then
        ...
        if [ "$security" == "reality" ]; then
            outbound=$(echo "$outbound" | jq --arg sni "$target_sni" --arg pbk "$pbk" --arg sid "$sid" --arg fp "${fp:-"firefox"}" \
            ...
            outbound=$(echo "$outbound" | jq --arg sni "$target_sni" --arg fp "${fp:-"firefox"}" --argjson ins "$is_insecure" \
...
_parse_anytls() {
    ...
    if [[ $link =~ $regex ]]; then
        ...
        if [ "$security" == "reality" ]; then
            jq -n --arg s "$server" --argjson p "$port" --arg pw "$password" --arg sni "${sni:-$server}" --arg pbk "$pbk" --arg sid "$sid" --arg fp "${fp:-firefox}" \
