...
历史 Mihomo 安装不再由项目管理：新版不会检查、调用或删除旧管理命令、内核、节点、证书和配置。用户可自行保留或另行人工处理。

菜单：**1** Emby 反代；**2** sing-box；**3** 检查脚本更新；**4** 只卸载统一入口；**0** 退出。

## 验证范围

参见 [回归记录](REGRESSION.md)。本地自动化检查覆盖发布完整性、更新回滚、路由状态和配置生成。当前没有可用 Linux VPS，实际 Reality 连接、Emby 视频播放和公网共享 443 尚未验收。
