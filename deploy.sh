...
Emby 前端根路径 / 返回无跳转欢迎页；新建和导入的 Emby 链路默认禁用 Web UI，客户端 API 与播放路径保留。
EOF
}
...
# Only single-server files emitted by our templates are eligible.
modernize_http2_file() {
    awk '
        /^[[:space:]]*server[[:space:]]*\{/ {servers++}
        /^[[:space:]]*http2[[:space:]]/ {
            if ($0 !~ /^[[:space:]]*http2[[:space:]]+on;[[:space:]]*(#.*)?$/) bad=1
            enabled++
        }
        {lines[NR]=$0}
        END {
            if (servers != 1 || bad || enabled > 1) exit 1
            for (i=1; i<=NR; i++) {
                line=lines[i]
                if (line ~ /^[[:space:]]*listen[[:space:]]+[^;]*[[:space:]]http2([[:space:]]|;)/) {
                    sub(/[[:space:]]+http2;/, ";", line)
                    sub(/[[:space:]]+http2[[:space:]]+/, " ", line)
                    print line
                    if (!enabled) {
                        match(line, /^[[:space:]]*/)
                        print substr(line, 1, RLENGTH) "http2 on;"
                        enabled=1
                    }
                } else print line
            }
...
fix_nginx_http2() (
    ...
    command -v nginx >/dev/null 2>&1 || { log_error '未安装 Nginx。'; exit 1; }
    nginx_supports_http2_directive || {
        log_info 'Nginx 低于 1.25.1 或版本未知，保留兼容的旧语法。'
        exit 0
    }
    nginx -t || exit 1
    local conf stage='' i
    ...
    trap '
        status=$?
        if ((${#config_tx_targets[@]})); then
            if rollback_config_changes; then
                restore_nginx_after_rollback
            else
                status=1
            fi
    ...
    for conf in /etc/nginx/conf.d/*.conf; do
        ...
        if ! grep -Fqx '# Generated by deploy-stream-domains.sh' "$conf" &&
           ! grep -Fqx '# Generated by singbox-lite Reality local origin' "$conf"; then
            continue
        fi
        grep -Eq '^[[:space:]]*listen[[:space:]]+[^;]*[[:space:]]http2([[:space:]]|;)' "$conf" || continue
        i=${#files[@]}
        if ! modernize_http2_file "$conf" > "$stage/$i"; then
            log_error "配置不是受支持的单 server 模板，未修改任何文件: $conf"
            exit 1
        fi
        files+=("$conf")
    done
    ((${#files[@]})) || { log_info '没有需要修复的受管 HTTP/2 配置。'; exit 0; }
    for i in "${!files[@]}"; do
        backup_file "${files[$i]}" || exit 1
        stage_file_install "$stage/$i" "${files[$i]}" || exit 1
    done
    test_and_reload_nginx || exit 1
    commit_config_changes
    log_success "已修复 ${#files[@]} 个配置；原文件保存在 $BACKUP_DIR。"
)
...
load_managed_link_for_edit() {
    ...
    emby_web_access=allow
    if grep -Fq '# nginxproxy-web-guard-begin' "$conf"; then emby_web_access=deny; fi
...
manage_existing_link() {
    ...
    case $action in
        ...
        2)
            ...
            ;;
        3|4)
            if [[ ${managed_link_modes[$index]} != emby ]]; then
                log_warn '此设置只适用于 Emby 反代，不修改本机服务反代。'
                return 0
            fi
            local web_action=deny
            [[ $action == 4 ]] && web_action=allow
            set_emby_web_access "${managed_link_configs[$index]}" "${managed_link_frontends[$index]}" "$web_action"
            ;;
        0) return 0 ;;
        *)
            log_error '无效输入。'
...
# 在 server rewrite 阶段拦截，避免推流的 ^~ location 绕过网页限制。
# 不使用 User-Agent；API、WebSocket、图片、字幕和视频路径仍按原配置转发。
emit_emby_web_guard() {
    local front_path="${1:-/}" prefix
    [[ $front_path == */ ]] || front_path="$front_path/"
    prefix=$(nginx_regex_escape "$front_path")
    printf '%s\n' '    # nginxproxy-web-guard-begin'
    printf '    if ($uri ~* "^(?:/|%s|/__emby_stream/[0-9]+/)(?:emby/)?(?:web(?:/|$)|index[.]html$)") { return 404; }\n' "$prefix"
    printf '%s\n' '    # nginxproxy-web-guard-end'
}
...
set_emby_web_access_locked() {
    ...
    [[ -f $conf && ! -L $conf ]] && grep -Fqx '# Generated by deploy-stream-domains.sh' "$conf" || {
        log_error '拒绝修改不属于本脚本的配置。'; return 1;
    }
    [[ $(grep -Ec '^server[[:space:]]*\{' "$conf") == 1 ]] || {
        log_error '配置包含多个或无法识别的 server 块，已保留。'; return 1;
    }
    parsed=$(parse_url "$frontend") || return 1
    IFS='|' read -r proto domain port path <<<"$parsed"
    tmp=$(mktemp); block=$(mktemp)
    if [[ $action == deny ]]; then emit_emby_web_guard "${path:-/}" > "$block"; fi
    awk -v block="$block" '
        /# nginxproxy-web-guard-begin/ {skip=1; next}
        /# nginxproxy-web-guard-end/ {skip=0; next}
        skip {next}
        {print}
        /^server[[:space:]]*\{/ {
    ...
    ' "$conf" > "$tmp"
    backup_file "$conf"
    stage_file_install "$tmp" "$conf"
    rm -f "$tmp" "$block"
    if ! test_and_reload_nginx; then
        rollback_config_changes || true
        restore_nginx_after_rollback
        log_error '配置校验或 reload 失败，已恢复原文件。'
        return 1
    fi
    commit_config_changes
    if [[ $action == deny ]]; then
        log_success 'Emby 网页入口已禁用。API 与播放路径保留；依赖网页界面的客户端也会被限制。'
    else
...
EOF
...

    {
        ...
        echo 'server {'
        if [[ $proxy_mode == emby && $emby_web_access == deny ]]; then emit_emby_web_guard "$front_path"; fi
        if [[ $frontend_mode == haproxy ]]; then
...
EOF
    ...
    # Reality handshake target without advertising the Emby Web UI.
    if [[ $front_path == / ]]; then
        cat >> "$tmp_conf" <<'EOF'
    location = / {
        default_type text/html;
        add_header Cache-Control "no-store";
        return 200 '<!doctype html><html><head><meta charset="utf-8"><title>Welcome</title></head><body><h1>Welcome</h1></body></html>';
    }
