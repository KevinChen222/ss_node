...
# 基础路径定义
export SCRIPT_VERSION="20-kevin.23"
...
SB_MIHOMO_ACME_RELOAD_CMD='if [ -s /run/nginx.pid ] && kill -0 "$(cat /run/nginx.pid)" 2>/dev/null; then nginx -s reload || exit 1; elif [ -d /run/systemd/system ] && command -v systemctl >/dev/null 2>&1; then systemctl start nginx || exit 1; elif command -v service >/dev/null 2>&1 && service nginx start; then :; else nginx || exit 1; fi; if [ -d /run/systemd/system ] && command -v systemctl >/dev/null 2>&1 && systemctl is-active mihomo >/dev/null 2>&1; then systemctl restart mihomo; elif command -v rc-service >/dev/null 2>&1 && rc-service mihomo status >/dev/null 2>&1; then rc-service mihomo restart; else :; fi'
...
_acme_ecc_cert_covers_domains() {
    local primary="$1" domain info cert_path
    shift
    info=$("$SB_ACME_SH" --info -d "$primary" --ecc 2>/dev/null || true)
    cert_path=$(printf '%s\n' "$info" | sed -n 's/^Le_RealFullChainPath=//p' | head -n 1)
    cert_path=${cert_path#\'}
    cert_path=${cert_path%\'}
    [ -n "$cert_path" ] && [ -s "$cert_path" ] || return 1
    for domain in "$primary" "$@"; do
        openssl x509 -in "$cert_path" -checkhost "$domain" -noout >/dev/null 2>&1 || return 1
    done
}

_issue_mihomo_certificate() {
    local primary="$1" cert_dir="${NGINX_CERT_DIR}/${1}" domain
    local -a domain_args=() force_args=()
    shift
    for domain in "$primary" "$@"; do domain_args+=(-d "$domain"); done
    if ! _acme_ecc_cert_is_usable "$primary" ||
       ! _acme_record_uses_webroot "$primary" ||
       ! _acme_ecc_cert_covers_domains "$primary" "$@"; then
        if "$SB_ACME_SH" --info -d "$primary" --ecc >/dev/null 2>&1; then
            force_args+=(--force)
            _warn "现有证书的域名、有效期或续期方式不符合要求，将重新签发。"
        fi
        _info "正在通过 HTTP-01 webroot 申请 Mihomo 证书: ${domain_args[*]}"
        "$SB_ACME_SH" --issue --server letsencrypt --webroot "$ACME_WEBROOT" \
            "${domain_args[@]}" --keylength ec-256 "${force_args[@]}" || return 1
    else
        _info "检测到仍有效且覆盖全部域名的 ECC 证书，将直接复用。"
    fi
    install -d -m 700 "$cert_dir" || return 1
    "$SB_ACME_SH" --install-cert -d "$primary" --ecc \
        --fullchain-file "$cert_dir/cert" \
        --key-file "$cert_dir/key" \
        --reloadcmd "$SB_MIHOMO_ACME_RELOAD_CMD" || return 1
    [ -s "$cert_dir/cert" ] && [ -s "$cert_dir/key" ] || return 1
    chmod 600 "$cert_dir/cert" "$cert_dir/key" 2>/dev/null || true
    printf 'MH_CERT_PATH=%s\nMH_KEY_PATH=%s\n' "$cert_dir/cert" "$cert_dir/key"
}

_prepare_mihomo_certificate() {
    local supplied normalized primary safe_primary challenge_conf tmp marker domain
    local -a domains=()
    [ "$#" -ge 1 ] || { _error "至少需要一个证书域名。"; return 2; }
    for supplied in "$@"; do
        normalized=${supplied,,}
        _is_valid_reality_domain "$normalized" || { _error "证书域名格式无效: ${supplied}"; return 1; }
        if ! printf '%s\n' "${domains[@]}" | grep -Fqx -- "$normalized"; then domains+=("$normalized"); fi
    done
    primary=${domains[0]}
    for domain in "${domains[@]}"; do
        if _target_uses_cloudflare "$domain"; then
            _warn "${domain} 当前经过 Cloudflare；将通过 CDN 回源完成 HTTP-01，请确保未拦截 /.well-known/acme-challenge/。"
        else
            _validate_local_origin_dns "$domain" || return 1
        fi
    done

    _pkg_install curl openssl tar ca-certificates coreutils || return 1
    _ensure_local_origin_dependencies || return 1
    safe_primary="${primary//[^a-z0-9.-]/_}"
    challenge_conf="${NGINX_CONF_DIR}/00-sb-mihomo-acme-${safe_primary}.conf"
    marker="# Generated by singbox-lite Mihomo ACME webroot"
    tmp=$(mktemp) || return 1
    {
        echo "$marker"
        echo 'server {'
        ...
        printf '    server_name'
        for domain in "${domains[@]}"; do printf ' %s' "$domain"; done
        printf ';\n'
    ...
    _issue_mihomo_certificate "${domains[@]}"
}

_remove_mihomo_certificate() {
    local domain="${1,,}" cert_path="$2" safe_domain challenge_conf marker cert_dir expected_cert
    local challenge_backup="" cert_home reference_path
    local -a reference_roots=()
    marker="# Generated by singbox-lite Mihomo ACME webroot"
    _is_valid_reality_domain "$domain" || { _error "证书域名格式无效: ${domain}"; return 2; }
    safe_domain="${domain//[^a-z0-9.-]/_}"
    challenge_conf="${NGINX_CONF_DIR}/00-sb-mihomo-acme-${safe_domain}.conf"
    cert_dir="${NGINX_CERT_DIR}/${domain}"
    expected_cert="${cert_dir}/cert"
    [ -n "$SB_ACME_HOME" ] && [ "$SB_ACME_HOME" != / ] || { _error "acme.sh 主目录校验失败，拒绝删除。"; return 1; }
    cert_home="${SB_ACME_HOME}/${domain}_ecc"

    if [ "$cert_path" != "$expected_cert" ]; then
        _warn "证书路径不属于 Mihomo 自动签发目录，已保留: ${cert_path}"
        printf 'MH_CERT_CLEANUP=unmanaged\n'
        return 0
    fi
    if [ ! -f "$challenge_conf" ] || ! grep -Fqx "$marker" "$challenge_conf" 2>/dev/null; then
        _warn "未找到此域名的 Mihomo 自动签发标记，证书及续期记录已保留。"
        printf 'MH_CERT_CLEANUP=unmanaged\n'
        return 0
    fi
    if [ -L "$challenge_conf" ] || [ -L "$cert_dir" ] || [ -L "$cert_path" ] || [ -L "$cert_home" ]; then
        _error "证书资源包含符号链接，拒绝自动删除。"
        return 1
    fi

    [ -d "$NGINX_CONF_DIR" ] && reference_roots+=("$NGINX_CONF_DIR")
    [ -f "${SINGBOX_DIR}/config.json" ] && reference_roots+=("${SINGBOX_DIR}/config.json")
    [ -f "${SINGBOX_DIR}/relay.json" ] && reference_roots+=("${SINGBOX_DIR}/relay.json")
    [ -f "/usr/local/etc/mihomo/config.yaml" ] && reference_roots+=("/usr/local/etc/mihomo/config.yaml")
    [ -f "/usr/local/etc/mihomo/nodes.json" ] && reference_roots+=("/usr/local/etc/mihomo/nodes.json")
    if [ "${#reference_roots[@]}" -gt 0 ]; then
        reference_path=$(grep -rslF -- "$cert_path" "${reference_roots[@]}" 2>/dev/null | head -n 1 || true)
        if [ -n "$reference_path" ]; then
            _warn "证书仍被其他配置引用，已保留证书和续期记录: ${reference_path}"
            printf 'MH_CERT_CLEANUP=shared\n'
            return 0
        fi
    fi

    challenge_backup=$(mktemp) || return 1
    cp -a -- "$challenge_conf" "$challenge_backup" || { rm -f -- "$challenge_backup"; return 1; }
    rm -f -- "$challenge_conf" || { rm -f -- "$challenge_backup"; return 1; }
    if command -v nginx >/dev/null 2>&1 && ! _sb_reload_or_start_nginx; then
        cp -a -- "$challenge_backup" "$challenge_conf" 2>/dev/null || true
        _sb_reload_or_start_nginx >/dev/null 2>&1 || true
        rm -f -- "$challenge_backup"
        _error "移除 ACME 域名配置后 Nginx 加载失败，已恢复配置。"
        return 1
    fi

    if [ -x "$SB_ACME_SH" ] && "$SB_ACME_SH" --info -d "$domain" --ecc >/dev/null 2>&1; then
        if ! "$SB_ACME_SH" --remove -d "$domain" --ecc >&2; then
            cp -a -- "$challenge_backup" "$challenge_conf" 2>/dev/null || true
            command -v nginx >/dev/null 2>&1 && _sb_reload_or_start_nginx >/dev/null 2>&1 || true
            rm -f -- "$challenge_backup"
            _error "删除 ${domain} 的 acme.sh 续期记录失败，证书文件已保留。"
            return 1
        fi
    fi
    rm -f -- "$challenge_backup"
    ...
    case "$cert_home" in
        "${SB_ACME_HOME}/"*_ecc)
            [ ! -e "$cert_home" ] || rm -rf -- "$cert_home" || return 1
            ;;
        *) _error "acme.sh 域名目录校验失败，拒绝删除。"; return 1 ;;
    esac
    rm -f -- "${cert_dir}/cert" "${cert_dir}/key" || return 1
    rmdir -- "$cert_dir" 2>/dev/null || true
    printf 'MH_CERT_CLEANUP=deleted\n'
}
...
_sni_router_assert_sni_available() (
    local sni="${1,,}" conflict
    _is_valid_reality_domain "$sni" || { _error "SNI 格式无效: ${sni}"; return 1; }
    _sni_router_lock || return 1
    _sni_router_init_state || return 1
    conflict=$(jq -r --arg sni "$sni" '
        if ((.reality.sni // "") | ascii_downcase) == $sni then
            "Reality: " + (.reality.tag // "unknown")
        elif .https_routes[$sni] != null then
            "HTTPS/Emby: " + (.https_routes[$sni].owner // "unknown")
        elif .tls_routes[$sni] != null then
            "TLS: " + (.tls_routes[$sni].tag // "unknown")
        else "" end
    ' "$SNI_ROUTER_STATE_FILE") || return 1
    if [ -n "$conflict" ]; then
        _error "完整域名 ${sni} 已被共享 443 路由占用（${conflict}）；请改用另一个子域名。"
        return 1
    fi
)
...
_sni_router_cli() {
    ...
    case "$action" in
        ...
        allocate-backend)
            ...
            ;;
        check-sni-free)
            [ "$#" -eq 1 ] || { _error "用法: sb sni-router check-sni-free SNI"; return 2; }
            _sni_router_assert_sni_available "$1"
            ;;
        register-reality)
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            issue-mihomo-certificate)
                ...
                exit $?
                ;;
            remove-mihomo-certificate)
                _check_root
                shift
                [ "$#" -eq 2 ] || { _error "用法: sb remove-mihomo-certificate <域名> <证书路径>"; exit 2; }
                _remove_mihomo_certificate "$1" "$2"
                exit $?
                ;;
            *)
                shift
